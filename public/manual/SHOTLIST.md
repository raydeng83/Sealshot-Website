...
- `capture-window.jpg` — the unified overlay with an element/window highlighted by boundary detection. ⟳ retake if the old shot shows the pre-0.7.0 overlay
- `capture-delayed.jpg` — the on-screen delayed-capture countdown. ✓ have
- `capture-scrolling.jpg` — scrolling capture in progress, or the stitched result. ✓ have
- `capture-live.jpg` — a Live Capture scene open in the editor: several windows as separate layers over the desktop wallpaper, one selected. 
...
- `record-prompt.jpg` — the Record prompt with system audio / microphone / cursor / countdown options.
- `recording-hud.png` — the floating recording HUD, hovered so Pause/Stop are visible.

## The editor (`editor.md`)
- `editor-overview.jpg` — a capture open in the editor: toolbar, canvas, right Info panel, strip. ⟳ retake (Info panel moved right in 0.7.0)
...
- `focus-crop.jpg` — the focus/crop viewfinder brackets over an image.
...
- `extract-data.jpg` — the Extract Structured Data window on a capture containing a table, CSV or Table tab active.

## Library & search (`library.md`)
- `library-grid.jpg` — the Library in grid view: sidebar with All Files / Recents / Collections (Favorites) / Trash visible. ⟳ retake (sidebar changed in 0.7.0)
- `library-strip.png` — the editor's recent-captures strip along the bottom (include a video with its play badge).
- `library-search.jpg` — a search query matching text *inside* a capture (OCR), results shown.
- `library-marquee.jpg` — a rubber-band selection rectangle drawn across several grid cards.
- `library-info-pane.png` — the Info panel showing name, summary, Smart Keywords, and tags. ⟳ retake (panel redesigned)
- `library-deleted.jpg` — the Trash section with Restore available.
...
- `security-setting.jpg` — Settings → Privacy & Security with Enhanced security toggled on (recovery code + auto-lock rows visible).
...
- `settings-general.jpg` — Settings → General (full page). ⟳ retake for 0.7.0 (tabs reorganized)
- `settings-shortcuts.jpg` — Settings → Shortcuts with the Capture / Recording / App cards. ⟳ retake (recording shortcuts added)
- `settings-permissions.jpg` — Settings → Permissions status list (Screen Recording, Microphone, Accessibility).
